...
* `sync/` – shared sync helpers
  * `cadence.ts` – `effectiveSyncEvery` + `computeNextSyncAt`. Applies to every platform, however it gets its data.
  * `resources.ts` – `resourcesFor`: module fields, else `config.resource`
  * `upsert.ts` – dedup + upsert against `External*Mapping` (today's `processIntegrationSync`). Named `upsert` rather than `ingest` so it can't be misread as having anything to do with Inngest.

There is **no `session/`**, and no `Session` type. The only thing platforms share about authentication is `authHeaders(creds)` — a pure function in `credentials.ts` — because the platforms that exist authenticate to different parties (`partner` to the partner, `ai` to n8n with a VIPER-global key) and the one that comes next authenticates with a browser cookie. A platform owns whatever client it needs.

There is also no `poll.ts` yet. The default `listChanged -> toCanonical -> upsert -> persist cursor` loop is real work that has to be written against a real ingest dispatch, and the first platform that pulls is the one that will write it. Every platform declares `sync` and owns its attempt end to end.
...
export interface ConnectorModule<TConfig = unknown, TCreds = unknown> {
  ...
  // How this platform syncs, end to end. Required: a puller loops and returns
  // the cursor it reached, a pusher fires one request and returns pending:true.
...
export interface ResourceModule<TCanonical, TRaw = unknown, TConfig = unknown> {
  // we pull from their platform. The module owns whatever client it needs —
  // core has no session abstraction to hand it.
  ...
  create?(c: CanonicalDraft<TCanonical>): Promise<PushResult<TRaw>>;
  update?(externalId: string, patch: Patch<TCanonical>): Promise<PushResult<TRaw>>;
...
There is no `changeSources`. Whether a platform pulls or hands off is decided at
runtime, by whether its strategy returns `pending: true` — a declaration would be
a second place for the same fact to live. Every enabled, due resource row is
scheduled; a platform with no module registered is scheduled too, so it records a
visible error rather than sitting `Pending` with nothing to look at.

Credentials reach exactly one place: `SyncCtx.creds`. They must **not** reach `toCanonical`, `apiUrlFor`, `webUrlFor`, or the upsert path — a mapper that can read credentials is one refactor away from logging them, and mappers have no use for them.
...
* **They speak ours** (`ai`, `partner`). Items arrive already in VIPER's shape — that's what "follows the VIPER standard" means, and for `ai` it's why n8n is handed `z.toJSONSchema(integrationAssetInputSchema)`. There's nothing to translate, so **every `ResourceModule` field is omitted**: no `listChanged`, no `toCanonical`, no `apiUrlFor`. Their `sync` strategy hands off, the callback validates against the existing `integration*InputSchema`, and `core/sync/upsert.ts` does the upsert.
...
export const genericConfigSchema = z.object({ resource: z.enum(ResourceType) });
...
export const resourcesFor = (p: ConnectorModule, config: unknown): ResourceType[] => {
  ...
  return fromModules.length ? fromModules : [genericConfigSchema.parse(config).resource];
};
```

`genericConfigSchema.parse` makes "a generic platform declares `config.resource`" a validated contract rather than a silent convention — it fails loudly instead of returning `[]` and never syncing. It runs against a row's stored JSON, which can be anything.

At the *type* level the same contract is structural: a generic platform's `configSchema` is built as `genericConfigSchema.extend({ … })`, so it cannot forget the field. There is no load-time assertion for it, because there is nothing left to assert.
...
* `session.ts` – how do we do auth? Fleet's own: an in-process TTL cookie cache,
  not a core abstraction. Core has no `Session` type — see Core, above.
...
export const teamplayFleet: ConnectorModule<Config, Creds> = {
  ...
  sync,          // Fleet pulls: loop its ResourceModules, persist the cursor
  workOrders,
  assets,
  notifications,
};
...
// TTL; there is no IntegrationSession table anymore. This is Fleet's own module:
// nothing in core knows about it, because no other platform authenticates this way.
export async function fleetCookie({ config, creds }): Promise<string> { /* … */ }
```
...
```ts
// genericConfigSchema contributes `resource`, so a platform with no
// ResourceModules cannot forget the field core resolves its resource from.
...
`sync.ts` – move `syncPartnerIntegration` here. POSTs `{ since: cursor ?? lastSuccessfulSync, page_size, callback }` and returns `{ pending: true }`; the partner pushes pages back to the callback. It POSTs with plain `fetch` to the one absolute URL in its config, authenticated with `authHeaders(ctx.creds)`. The callback token has to become multi-use (or per-page) before `max_pages: 1` can be lifted.

Both `ai` and `partner` share `core/callback.ts` — that's the only thing their strategies have in common. Note they authenticate to *different parties*: `partner` to the partner with the integration's own credentials, `ai` to n8n with a VIPER-global `N8N_KEY` while forwarding the integration's credentials as payload. That difference is why there is no shared session abstraction.
...
* **`syncAllIntegrations`** — cron `*/5`. Fan out one `integration/sync.requested { integrationId, resource }` per row where the integration is `enabled`, the resource sync is `enabled`, and `nextSyncAt IS NULL OR nextSyncAt <= now()`. Three separate facts, three separate columns — no nulls doing double duty. There is no platform filter: a platform with no module registered is fanned out too, so it records a visible error instead of sitting `Pending` forever.
* **`syncIntegration`** — `concurrency: { key: 'event.data.integrationId + event.data.resource' }` so a manual *Sync Now* can't overlap the cron. **Four** steps, none platform-aware:
  1. load the integration — non-secret JSON only. `omit: { credentials: true }`, because a step's return value is shipped to and memoized by the Inngest service.
  2. advance `nextSyncAt` and stamp `lastAttemptAt` — **before** the work, so a crash costs one cycle instead of wedging the row
  3. resolve the platform, re-read and decrypt credentials *in-process*, build the `SyncCtx`, and run `module.sync(ctx)` — all in **one** step. `step.run` memoizes its return as JSON, so a closure or a module cannot cross a step boundary; anything non-serializable has to be created and consumed inside a single one.
...
Everything that differs between platforms — pulling pages vs. hand-off-to-n8n vs. ask-the-partner-to-push — lives in the strategy, in the platform's own folder. Adding a platform means adding a directory; it never means editing an Inngest function.
...
* Webhook-driven platforms — nothing declares one yet, and the cron currently schedules every enabled row, so one would get pointless ticks until that's revisited
* Multi-page PARTNER pushes are blocked on the single-use callback token
* Should the inbox be a platform? Resend as a webhook-driven `Integration` would put email identity on a mapping like everything else, and `SourceRecord.externalId` could go away
