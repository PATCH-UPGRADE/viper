...
import { ArtifactType, AuthType, ResourceType, SyncStatusEnum } from "@/generated/prisma";
import prisma from "@/lib/db";
import type { ArtifactWrapperWithUrls } from "@/lib/schemas";
import { AUTH_TOKEN, authHeader, BASE_URL, generateCPE, jsonHeader, setupMockIntegration } from "./test-config";
...
describe("DeviceArtifacts Endpoint (/deviceArtifacts)", () => {
  ...
  //   const noDeviceArtifacts = { ...deviceArtifactsIntegrationPayload, items: [] };
  //   const createRes = await request(BASE_URL)
  ...
  it("create DeviceArtifact uploadIntegration endpoint int test", async () => {
    ...
    const mapping1 = await prisma.externalDeviceArtifactMapping.findFirstOrThrow(
      {
        where: {
    ...
    console.log(foundDeviceArtifact1);
    ...
    const mapping2 = await prisma.externalDeviceArtifactMapping.findFirstOrThrow({
      where: {
