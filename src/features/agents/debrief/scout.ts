...
/**
 * The scout binds ONLY query_platform_data and record_finding — deliberately
 * not `buildAgentTools`.
 *
...
function buildScoutGraph() {
  const tools = [
    makeQueryPlatformDataTool(AUTOMATION_USER_ID),
    recordFindingTool,
  ];
...
export async function runDebriefScout(): Promise<string> {
  ...
  const findings = await enrichFindings(extractFindings(result.messages));
  ...
  if (findings.length === 0) {
    throw new Error("Debrief scout recorded no findings");
  }

  return renderFindings(findings).slice(0, MAX_FINDINGS_CHARS);
}
