...
// Rendered only while nothing is in flight, so the button needs no busy state:
// pressing it flips the card to PendingBody on the next render.
const FailedBody = ({ onRetry }: { onRetry: () => void }) => (
...
export const DebriefCard = () => {
  ...
  return (
    <Card className="gap-0 py-0 shadow-none">
      <Collapsible open={open} onOpenChange={setOpen} className="group/debrief">
        <div className="flex items-start justify-between gap-4 p-5">
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            <CollapsibleTrigger asChild>
              ...
              >
                <ChevronDownIcon
                  className="size-4 transition-transform group-data-[state=closed]/debrief:-rotate-90"
                  aria-hidden="true"
                />
              </Button>
            </CollapsibleTrigger>
        ...
        <CollapsibleContent className="border-t">
          {hasBrief ? (
            ...
            <FailedBody onRetry={() => regenerate.mutate()} />
          )}
