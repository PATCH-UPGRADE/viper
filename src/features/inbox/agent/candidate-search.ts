...
async function searchDeviceGroupMatching(
  ...
  const terms = [extracted.manufacturer, extracted.modelName].filter(
...
async function searchRemediation(
  extracted: ExtractedRemediation,
  ...
  if (extracted.description) {
    const insensitive = {
      contains: extracted.description,
...
export async function searchCandidates(
  ...
  const [deviceGroups, vulnerabilities, remediations, assets] =
    await Promise.all([
      Promise.all(
      ...
      Promise.all(
        extracted.vulnerabilities.map(async (v) => ({
          extracted: v,
          matches: await searchVulnerability(v),
        })),
      ),
      Promise.all(
        extracted.remediations.map(async (r) => ({
          extracted: r,
          matches: await searchRemediation(r),
        })),
      ),
      Promise.all(
        extracted.assets.map(async (a) => ({
          extracted: a,
