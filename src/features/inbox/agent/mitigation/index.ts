...
- cards.rollback is the exception to that brevity, but only just: at most two sentences and under 200 characters on how easily this plan can be undone once it has been applied — what has to be reverted, roughly how long it takes, and whether the manufacturer has to be on site. Ground it in the plan's own work orders: a network or configuration change is usually quick to reverse, a firmware update usually is not. Say "Uncertain" rather than guessing.
