...
- always fill cards.residual_risk_note: it explains the residual_risk judgement in one or two sentences under 200 characters — what stays exposed and until when, or why the path is closed. It is shown after the level as "Residual risk — Low. <note>", so do not open by repeating the level word.
- cards.rollback_level classifies how hard this plan is to undo once applied — Easy, Moderate, Hard or Uncertain. Ground it in the plan's own work orders: a network or configuration change staff can revert themselves is usually Easy, a firmware update or vendor patch is usually Hard. Choose Uncertain rather than guessing.
- cards.rollback_summary is the same judgement in one short clause under 80 characters, rendered inline right after the level as "Easy — <summary>", so write it to read on from the level (e.g. "reverts from the switch config in ~15 min, no vendor needed"). Do not repeat the level word.
- cards.rollback explains that level in at most two sentences and under 200 characters: what has to be reverted, roughly how long it takes, and whether the manufacturer has to be on site. Do not open by repeating the level word — it is displayed separately.
...
- performedBy says who does the work, and it decides whether the order is sent to the vendor's own service platform or tracked here. Set 'vendor' only when the hospital cannot do the work itself — firmware, hardware, calibration, anything needing the vendor's service credentials or an engineer on site. Set 'hospital' for account changes, network rules, configuration, monitoring and policy, EVEN when the device is under a vendor service contract. Split the work rather than stretching one order: a firmware update the vendor applies and an account audit your staff run are two work orders, not one.
