...
const questionSchema = z.object({
  ...
  audience: z
    ...
    .describe(
      "Who could answer this. MANUFACTURER when the company that built the device could answer from product knowledge alone - whether a component ships in the product, whether the flaw is reachable by design, whether a fix exists. VENDOR when it depends on how this hospital's own units are deployed, configured or serviced, which only whoever manages them would know. This is about what kind of party could answer. Check the issue's 'Managed by' line.",
    ),
});
