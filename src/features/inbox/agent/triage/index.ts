...
- likelihood: How likely exploitation is at this hospital — a plain-words judgment first, then the single reason that matters most, in everyday terms. This renders in a small card, so it MUST be one sentence of about 25 words and never more than two short ones. Pick the strongest signal from the evidence in the context (CVSS, EPSS, CISA KEV status, exploit availability, which devices are already confirmed unaffected); do not work through them all. Write whatever judgment the evidence supports. Good (severe): "Very likely to be exploited — attackers are already using this flaw, and it needs no password." Good (mild): "Unlikely to be exploited here — an attacker would need to be on the hospital network first." Bad, both jargon and too long: "Unauthenticated network RCE · PoC exploit code exists · EPSS 42% · On CISA KEV". Never invent numbers; never lead with raw scores.
