...
import { grabSessionCookie, SessionLoginConfig } from "./teamplay-fleet/capture";
...
export class IntegrationSessionClient {
    ...
    constructor(host: string, username: string, password: string, loginConfig: SessionLoginConfig) {
        this.host = host;
    ...
    async fetchWithSession(url: string): Promise<Response> {
        const cached = await prisma.integrationSession.findUnique({ where: { host: this.host } });
        const headers: Record<string, string> = {};

        if (cached) headers[cached.header] = cached.value;
        const res = await fetch(url, { headers, signal: AbortSignal.timeout(REAUTH_TIMEOUT) });
    ...
    private async reauth() {
        ...
        const session = await grabSessionCookie(this.loginConfig, userName, password);

        await prisma.integrationSession.upsert({
            where: { host: this.host},
            create: {host: this.host, header: session.header, value: session.value, expiresAt: session.expiresAt},
            update: { header: session.header, value: session.value, expiresAt: session.expiresAt}
        });
