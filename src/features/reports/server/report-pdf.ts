...
export async function renderReportPdf(
  ...
  const block = (b: ReportBlock) => {
    switch (b.type) {
      case "heading":
        ...
        break;
      case "paragraph":
        draw(spanText(b.spans), { gap: 8 });
        break;
      case "listItem":
        draw(`${b.marker}  ${spanText(b.spans)}`, { indent: 12, gap: 4 });
        break;
      case "table":
  ...
  draw(title?.trim() || "Report", { font: bold, size: 24, gap: 2 });
  draw(`Generated by Viper · ${new Date().toISOString().slice(0, 10)}`, {
    size: 9,
    gap: 14,
  });
