import { chromium } from "playwright-core";
    ...
    for (let attempt = 1; attempt <= maxAttempt; attempt++) {
      ...
      try {
        ...
        if (config.cookieBannerAcceptSelector) {
          await page.click(config.cookieBannerAcceptSelector, { timeout: 5_000 }).catch(() => {});
        }
        ...
        const [authProbeResponse] = await Promise.all([
          page.waitForResponse((res) => res.url().startsWith(config.authProbeUrl)),
          page.click(config.submitSelector),
        ]);

        if (authProbeResponse.status() === 401 || authProbeResponse.status() === 403 || !authProbeResponse.ok()) {
        ...
        const expires = cookies.map((c) => c.expires).filter((e): e is number => typeof e === "number" && e > 0);
        const expiresAt = expires.length > 0 ? new Date(Math.min(...expires) * 1000) : null;
  ...
  export const FLEET_LOGIN_CONFIG: SessionLoginConfig = {
    ...
    cookieBannerAcceptSelector: "#CybotCookiebotDialogBodyLevelButtonLevelOptinDeclineAll",
    ...
    authProbeUrl: "https://fleet.siemens-healthineers.com/rest/v1/security-advisories/active",
  };
